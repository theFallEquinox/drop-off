...
package net.thefallequinox.dropoff.logic;
...
import net.thefallequinox.dropoff.DropoffConfig;
import net.thefallequinox.dropoff.helpers.FakeAboveShapeContext;
import net.thefallequinox.dropoff.helpers.FallingGroupInterface;

public class BlockHandler {

	public static float getBlastResistance(Block block) {
		return block.getBlastResistance() * DropoffConfig.getBlastResistanceScaling();
	}

	public static float getHardness(Block block) {
		return block.getHardness() * DropoffConfig.getHardnessScaling();
	}
	...
	public static boolean isBuoyant(BlockState state) {
		...
		return state.isLavaIgnitable() || block.getSlipperiness() > 0.6f || state.isIn(Dropoff.BUOYANT);
	}

    public static boolean canHang(BlockState state) {
		Block block = state.getBlock();
        return state.getLuminance() > 0 || (state.getSoundGroup() == BlockSoundGroup.GLASS && block.getSlipperiness() <= 0.6F) || state.isIn(Dropoff.HANGS);
    }

	public static boolean isProtected(BlockState state) {
		return state.getSoundGroup() == BlockSoundGroup.GLASS || state.isIn(Dropoff.PROTECTED);
	}

	public static boolean canFloat(BlockState state) {
		Block block = state.getBlock();
		return getBlastResistance(block) >= 1200 || state.isIn(Dropoff.FLOATS);
	}
    ...
    public static int getReach(BlockState state) {
        ...
        if (isProtected(state)) {
            resis = Math.max(resis, DropoffConfig.getProtectedReachMinimum());
        }
    ...
    public static void fall(BlockState state, World world, BlockPos pos, FallingBlockGroup fallingGroup) {
        ...
        if (state.isIn(Dropoff.USE_DEFAULT_STATE)) {
		...
		float hurtAmount = block.getHardness() * 0.4F * DropoffConfig.getCaveInDamageScaling();
    ...
    public static boolean updateNeighbors(World world, BlockPos pos, FallingBlockGroup fallingGroup, int updates) {
		if (updates > DropoffConfig.getMaxBlockUpdates()) return false;
